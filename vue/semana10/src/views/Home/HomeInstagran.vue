<template>
    ...
    <v-form class="d-flex" ref="form" @submit.prevent="handleSubmit">
        <v-text-field class="w-25 me-5" label="Nome" variant="outlined" :rules="[v => !!v || 'Nome é obrigatório!']"
            v-model="post.usuario" required></v-text-field>

        <v-text-field class="w-50" label="Imagem" variant="outlined" :rules="[v => !!v || 'Imagem é obrigatória!']"
            v-model="post.imagem" required></v-text-field>
    ...
    <div class="d-flex flex-wrap w-100">
        <v-card v-for="(item, index) in postsList" :key="index" variant="tonal" class="w-25 ma-2" :title="item.usuario">
